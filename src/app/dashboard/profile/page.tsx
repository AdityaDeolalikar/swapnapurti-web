...
import { FaUser, FaEnvelope, FaPhone, FaMapMarkerAlt, FaCamera, FaEdit, FaCalendarAlt, FaTint, FaBuilding, FaBriefcase, FaLock, FaTimes, FaPlus, FaStar, FaStarHalfAlt, FaRegStar, FaPaperPlane, FaGraduationCap, FaTools, FaBusinessTime, FaCalendarCheck } from 'react-icons/fa';
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="lg:order-2 lg:col-span-1 order-1 space-y-4 sm:space-y-6">
          {/* Profile Picture */}
            ...
            <div className="flex flex-col items-center">
              <div className="relative group">
                <img
                  src={previewUrl}
                  alt={user.name}
                  className="w-24 sm:w-32 h-24 sm:h-32 rounded-full object-cover ring-4 ring-blue-50"
                />
                {isEditing && (
                  <>
                    <input
                    ...
                    <button 
                      ...
                      className="absolute bottom-0 right-0 p-1.5 sm:p-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-colors duration-200"
                    >
              ...
              <h2 className="mt-3 sm:mt-4 text-lg sm:text-xl font-semibold text-gray-800">{user.name}</h2>
              <p className="text-sm sm:text-base text-gray-500">{user.occupation}</p>
              <p className="text-xs sm:text-sm text-gray-500 mt-2">Member since {user.joinDate}</p>
            </div>
          </div>
        </div>

        {/* Main Profile Content */}
        <div className="lg:order-1 lg:col-span-2 order-2 space-y-4 sm:space-y-6">
          {/* Basic Info */}
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 order-1">
            <div className="flex justify-between items-center mb-4 sm:mb-6">
              ...
              {isEditing && (
                <button
                  onClick={handleSave}
                  className="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 text-white text-sm sm:text-base rounded-lg hover:bg-blue-700 transition-colors duration-200 flex items-center gap-2"
                >
                  Save Changes
                </button>
              )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-lg">
                  ...
                  <span className="text-gray-800">{user.bloodGroup}</span>
                </div>
              </div>

              <div className="md:col-span-2">
                <p className="text-sm text-gray-500 italic">Note: Basic information can only be updated by contacting support.</p>
              </div>
            </div>
          </div>

          {/* Personal Details */}
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 order-2">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 sm:mb-6">
              ...
              <button
                ...
                className="w-full sm:w-auto px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 text-white text-sm sm:text-base rounded-lg hover:bg-blue-700 transition-colors duration-200 flex items-center justify-center sm:justify-start gap-2"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-600 mb-2">Permanent Address</label>
                {isEditing ? (
                  <textarea
                  ...
                  <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-lg">
                    ...
                    <span className="text-gray-800">{user.permanentAddress}</span>
                  </div>
                )}
              </div>

              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-600 mb-2">Temporary Address</label>
                {isEditing ? (
                  <textarea
                  ...
                  <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-lg">
                    <FaMapMarkerAlt className="text-gray-400" />
                    <span className="text-gray-800">{user.temporaryAddress}</span>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 rounded-lg">
                    <FaPhone className="text-gray-400" />
                    <span className="text-gray-800">{user.emergencyPhone}</span>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 order-3">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 sm:mb-6">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-800">Interests & Opportunities</h2>
        ...
        {/* Additional Sections */}
        <div className="lg:order-3 lg:col-span-1 order-3 space-y-4 sm:space-y-6">
          {/* Performance Ratings */}
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 order-1">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-4 sm:mb-6">Performance Ratings</h2>
            <div className="space-y-3 sm:space-y-4">
              <div className="flex flex-col gap-1">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-600">Event Participation</span>
                  <span className="text-sm font-medium text-gray-900">{user.ratings.eventParticipation}</span>
                </div>
                <div className="flex gap-1">
                  {renderStarRating(user.ratings.eventParticipation)}
                </div>
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div 
                    className="bg-yellow-400 h-1.5 rounded-full" 
                    style={{ width: `${(user.ratings.eventParticipation / 5) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-medium text-gray-900">{user.ratings.communication}</span>
                </div>
                <div className="flex gap-1">
                  {renderStarRating(user.ratings.communication)}
                </div>
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div 
                    className="bg-yellow-400 h-1.5 rounded-full" 
                    style={{ width: `${(user.ratings.communication / 5) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-gray-600">Teamwork</span>
                  <span className="text-sm font-medium text-gray-900">{user.ratings.teamwork}</span>
                </div>
                <div className="flex gap-1">
                  {renderStarRating(user.ratings.teamwork)}
                </div>
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div 
                    className="bg-yellow-400 h-1.5 rounded-full" 
                    style={{ width: `${(user.ratings.teamwork / 5) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-medium text-gray-900">{user.ratings.leadership}</span>
                </div>
                <div className="flex gap-1">
                  {renderStarRating(user.ratings.leadership)}
                </div>
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div 
                    className="bg-yellow-400 h-1.5 rounded-full" 
                    style={{ width: `${(user.ratings.leadership / 5) * 100}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className="pt-4 mt-4 border-t border-gray-200">
                <div className="flex flex-col gap-1">
                  <div className="flex justify-between items-center">
                    <span className="text-sm font-bold text-gray-900">Overall Rating</span>
                    <span className="text-sm font-bold text-gray-900">{user.ratings.overall}</span>
                  </div>
                  <div className="flex gap-1">
                    {renderStarRating(user.ratings.overall)}
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-yellow-400 h-2 rounded-full" 
                      style={{ width: `${(user.ratings.overall / 5) * 100}%` }}
                    ></div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 order-2">
            <div className="flex items-center gap-2 sm:gap-3 mb-4">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-800">Message for Team</h2>
              <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs">Team Communication</span>
            </div>
            ...
            <div className="relative">
              <textarea
                ...
                className="w-full px-3 sm:px-4 py-2 sm:py-3 pr-20 sm:pr-24 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-sm sm:text-base"
                rows={4}
              />
          ...
          {/* Account Settings */}
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 order-3">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-4">Account Settings</h2>
            <div className="space-y-2 sm:space-y-3">
              <button className="w-full text-left px-3 sm:px-4 py-2 text-sm sm:text-base text-gray-600 hover:bg-gray-50 rounded-lg transition-colors duration-200 flex items-center gap-2">
